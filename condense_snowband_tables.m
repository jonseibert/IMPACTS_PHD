...
% 3) AIR Ens Mean of Per-Member Mean Match Distance (m,m*pt)
% 4) CONV Ens Mean of Per-Member Mean Match Distance (m,m*pt)
% 5) Ens Mean Difference in Per-Member Mean Match Distances (m,m*pt)
% 6) No. Total Instances [Mem x Thresh/Time]
% 7) % Instances AIR Closer (%)
num_vars = 7;
...
for dir_idx = 1:num_directions
    for thresh_idx = 1:num_thresholds
        ...
        stats_thresh_w0(dir_idx,thresh_idx,6) = length(dif);
        stats_thresh_w0(dir_idx,thresh_idx,7) = length(dif(dif <= 0))/length(dif);
    end
    for time_idx = 1:num_times
        ...
        stats_time_w0(dir_idx,time_idx,6) = length(dif);
        stats_time_w0(dir_idx,time_idx,7) = length(dif(dif <= 0))/length(dif);
    end
end
...
for dir_idx = 1:num_directions
    for thresh_idx = 1:num_thresholds
        ...
        % Should this be "Sum"? Yes.
        dif = T_C{rows_c,"Sum"}-T_D{rows_d,"Sum"};
        stats_thresh_w1(dir_idx,thresh_idx,5) = mean(dif,"omitnan");
        stats_thresh_w1(dir_idx,thresh_idx,6) = length(dif);
        stats_thresh_w1(dir_idx,thresh_idx,7) = length(dif(dif <= 0))/length(dif);
    end
    for time_idx = 1:num_times
        ...
        stats_time_w1(dir_idx,time_idx,3) = mean(T_C{rows_c,"Sum"},"omitnan");
        stats_time_w1(dir_idx,time_idx,4) = mean(T_D{rows_d,"Sum"},"omitnan");
        dif = T_C{rows_c,"Sum"}-T_D{rows_d,"Sum"};
        stats_time_w1(dir_idx,time_idx,5) = mean(dif,"omitnan");
        stats_time_w1(dir_idx,time_idx,6) = length(dif);
        stats_time_w1(dir_idx,time_idx,7) = length(dif(dif <= 0))/length(dif);
    end
end
...
% 6) No. Instances (IC)
% 7) % Instances AIR Closer (PC_A)
stats = stats_thresh_w0;
subname = "finalstats_thresh_w0";
row_idx = 1;
for dir_idx = 1:2
    for thresh_idx = 1:num_thresholds
        for col_idx = 1:num_vars
