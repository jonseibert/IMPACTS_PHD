%% Experimentally convert 3D EXRAD wind fields into partial-3D radial velocity fields from a single radar point
%  For now: KBUF
% 
% Jon Seibert
% 16 Nov 2021
...
output_path = "output/radial_velocity/exrad";
...
%for angle_idx = 1:length(elevation_angles)
    ...
    xlabel('Longitude','FontSize',label_font_size);
    ylabel('Latitude','FontSize',label_font_size);
    title(sprintf("Radial Velocity: EXRAD | %s | %g* elevation | KBUF",timestamp,angle),'FontSize',title_font_size);
...
%% TESTING (exrad_wind_conversion.m)

% Lat N / LON W (deg-min-sec) KBUF: 425656 / 0784412
lat_radar = 42.9489;
lon_radar = 78.7367;
...
% Compute azimuth angle
lat_target = latitude(y,x,z);
lon_target = longitude(y,x,z);
height_target = vertical_grid(z);
az = azimuth('gc',lat_radar,lon_radar,lat_target,lon_target); % Great circle azimuth
az_math = -1*(az - 90)*(pi/180); % convert azimuth to counterclockwise from East, in radians [math form]
...
%Vr = u*sin(az_math)*cosd(elev) + v*cos(az_math)*cosd(elev) + w*sind(elev);

Vr_test = u_full*sin(az_math)*cosd(angle) + v_full*cos(az_math)*cosd(angle) + w_full*sind(angle);
...
% Plot
figure('Position',[0 200 2100 620]);
h = pcolor(longitude(:,:,1),latitude(:,:,1),Vr_test(:,:,z));
hold on;
set(h, 'EdgeColor', 'none');
shading interp;
c = colorbar('FontSize',axes_font_size);
...
saveas(h,sprintf("%s/radial_test_full_z%d_e%d_c%s.png",output_path,z,angle_idx,cmap_name));
close all;