...
function output = neighborhood_prob_3 (binary,lon,lat,roi)
    ...
    for i_c = 1:dims(1) % Centerpoint coordinates
        for j_c = 1:dims(2)
            ...
            for i_t = il_lim:iu_lim % Target point coordinates
                ...
                for j_t = jl_lim:ju_lim
                    ...
                    if(d > prev_dist && d > roi) % If there are no more gridpoints within the radius on this cycle, skip the rest
                        break;
                    end
        ...
        %    fprintf("[NP3] Progress: 75%%\n");
        %elseif(i_c > q2 && i_c <= q2 + 1)
        %    fprintf("[NP3] Progress: 50%%\n");
        %elseif(i_c > q1 && i_c <= q1 + 1)
        %    fprintf("[NP3] Progress: 25%%\n");
        %elseif(i_c == 1)
        %    eta = toc*dims(1)/60;
        %    fprintf("[NP3] Estimated time to completion: %.1f minutes\n", eta);
        %end
