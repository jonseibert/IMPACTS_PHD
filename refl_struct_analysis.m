    ...
    for member_idx = 1:(num_members+1)
        ...
        for exp_idx = 1:2
            ...
            for slice_idx = 1:3
                ...
                if(y_axis)
                    beam_width_intersection_pressure = zeros(num_radars,2,ylen_wrf); % TESTING
                    ...
                    for rad_idx = 1:num_radars
                        ...
                        if(plot_beam_width)
                            ...
                            rad_lon = lon_deg(radars_used(rad_idx));
                            rad_lat = lat_deg(radars_used(rad_idx));
                            rad_elev = elevations(radars_used(rad_idx));
                            rad_th = tower_heights(radars_used(rad_idx));
                        end
                        for y = 1:ylen_wrf
                            % TESTING
                            if(plot_beam_width)
                                wrf_lon_val = slice_lon;
                                wrf_lat_val = linear_interpolate(lon_wrf(y,idx_a),lon_wrf(y,idx_b),slice_lon,lat_wrf(y,idx_a),lat_wrf(y,idx_b));
                                D = haversine_distance(rad_lon,rad_lat,wrf_lon_val,wrf_lat_val);
                                R = D/(cosd(beam_angle_u));
                                Hu = sqrt(R^2 + ((4/3)*Re)^2 + 2*R*(4/3)*Re*sind(beam_angle_u)) - (4/3)*Re + rad_elev + rad_th; % Keenan version adjusted for m
                                R = D/(cosd(beam_angle_l));
                                Hl = sqrt(R^2 + ((4/3)*Re)^2 + 2*R*(4/3)*Re*sind(beam_angle_l)) - (4/3)*Re + rad_elev + rad_th; % Keenan version adjusted for m
                                ...
                                for z = 1:zlen_wrf
                                    ...
                                    if(model_height_interp(y,z) > rad_height)
                                        beam_width_intersection_pressure(rad_idx,1,y) = linear_interpolate(model_height_interp(y,max(z-1,1)),model_height_interp(y,z),rad_height,pressure_slice(y,max(z-1,1)),pressure_slice(y,z));
                                        break;
                                    end
                                end
                                rad_height = Hl;
                                for z = 1:zlen_wrf
                                    ...
                                    data_interp(y,z) = linear_interpolate(lon_wrf(y,idx_a),lon_wrf(y,idx_b),slice_lon,data(y,idx_a,z),data(y,idx_b,z));
                                    if(model_height_interp(y,z) > rad_height)
                                        beam_width_intersection_pressure(rad_idx,2,y) = linear_interpolate(model_height_interp(y,max(z-1,1)),model_height_interp(y,z),rad_height,pressure_slice(y,max(z-1,1)),pressure_slice(y,z));
                                        break;
                                    end
                                end
                            end
                    ...
                    beam_width_intersection_pressure = zeros(num_radars,2,xlen_wrf); % TESTING
                    ...
                    for rad_idx = 1:num_radars
                        ...
                        for x = 1:xlen_wrf
                            if(plot_beam_width)
                                wrf_lon_val = linear_interpolate(lat_wrf(idx_a,x),lat_wrf(idx_b,x),slice_lat,lon_wrf(idx_a,x),lon_wrf(idx_b,x));
                                wrf_lat_val = slice_lat;
                                D = haversine_distance(rad_lon,rad_lat,wrf_lon_val,wrf_lat_val);
                                R = D/(cosd(beam_angle_u));
                                Hu = sqrt(R^2 + ((4/3)*Re)^2 + 2*R*(4/3)*Re*sind(beam_angle_u)) - (4/3)*Re + rad_elev + rad_th; % Keenan version adjusted for m
                                R = D/(cosd(beam_angle_l));
                                ...
                                for z = 1:zlen_wrf
                                    ...
                                    if(model_height_interp(x,z) > rad_height)
                                        beam_width_intersection_pressure(rad_idx,1,x) = linear_interpolate(model_height_interp(x,max(z-1,1)),model_height_interp(x,z),rad_height,pressure_slice(x,max(z-1,1)),pressure_slice(x,z));
                                        break;
                                    end
                                end
                                rad_height = Hl;
                                for z = 1:zlen_wrf
                                    ...
                                    data_interp(x,z) = linear_interpolate(lat_wrf(idx_a,x),lat_wrf(idx_b,x),slice_lat,data(idx_a,x,z),data(idx_b,x,z));
                                    if(model_height_interp(x,z) > rad_height)
                                        beam_width_intersection_pressure(rad_idx,2,x) = linear_interpolate(model_height_interp(x,max(z-1,1)),model_height_interp(x,z),rad_height,pressure_slice(x,max(z-1,1)),pressure_slice(x,z));
                                        break;
                                    end
                                end
                            end
                ...
                if(plot_all_beams)
                    if(y_axis)
                        for rad_idx = 1:num_radars
                            ...
                            if(plot_beam_width)
                                h = plot(slice_lat_list,squeeze(beam_width_intersection_pressure(rad_idx,1,:)),'Color','red','LineWidth',2,'LineStyle','-');
                                h = plot(slice_lat_list,squeeze(beam_width_intersection_pressure(rad_idx,2,:)),'Color','blue','LineWidth',2,'LineStyle','-');
                            end
                        end
                        ...
                        for rad_idx = 1:num_radars
                            ...
                            if(plot_beam_width)
                                h = plot(slice_lon_list,squeeze(beam_width_intersection_pressure(rad_idx,1,:)),'Color','red','LineWidth',2,'LineStyle','-');
                                h = plot(slice_lon_list,squeeze(beam_width_intersection_pressure(rad_idx,2,:)),'Color','blue','LineWidth',2,'LineStyle','-');
                            end
                        end
                    end
                end
                ...
                if(y_axis)
                    ...
                    ylabel('Pressure (hPa)','FontSize',label_font_size);
                else
            ...
            if(plot_height_maps)
                ...
                for plot_idx = 1:3
                    ...
                    if(use_contours)
                        ...
                        c = colorbar('FontSize',axes_font_size); % Make colorbar
                        colormap(cmap); % Set colors
                        ylabel(c,'Pressure at height of maximum dBZ (hPa)');
                    end
                    ...
                    if(plot_type == "raw") %title_format_raw =   '[%s|d0%d|%s] Raw: %s(%s)|%s'; % exp name, domain, member #/mean, data type, units, timestamp
                        title(sprintf('[%s|d0%d|%s] Height of Maximum Refl (hPa) | %s',exp_name_plot,domain,member_string,timestamp_title),'FontSize',title_font_size); 
                    elseif(plot_type == "dif")
                        title(sprintf('[%s-%s|d0%d|%s] Dif: Heights of Maximum Refl (hPa) | %s',exp_name_a,exp_name_b,domain,member_string,timestamp_title),'FontSize',title_font_size); 
                    end
                    xlabel('Longitude','FontSize',label_font_size);
