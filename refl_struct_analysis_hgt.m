...
load(sprintf('%s/%s',intermediate_path,'nas_slice_dims.mat'),'lon_list_box','lat_list_box','hgt_list_box','dims_list_box');
closest_hours = [14 15 15 15];
...
for time_idx = 1:4
    ...
    hour = closest_hours(time_idx);
    min_height = 0;
    max_height = hgt_list_box(time_idx,1);
    w_limit = min(lon_list_box(time_idx,1:dims_list_box(time_idx,2)));
    e_limit = max(lon_list_box(time_idx,1:dims_list_box(time_idx,2)));
    ...
    for member_idx = 1:(num_members+1)
        ...
        for exp_idx = 1:2
            ...
            slice_idx = time_idx;
                switch slice_idx
                    case 1
                         % Slice 1: X-Z, along longitude:
                        slice_lat = lat_list_box(time_idx,1);
                        y_axis = 0;
                    case 2
                ...
                if(y_axis) % Slice is along y-axis
                    ...
                    x_dir = 1;
                    if(lon_wrf(y_mid,2) < lon_wrf(y_mid,1))
                        x_dir = -1;
                    end
                    for x = 1:xlen_wrf
                        if(lon_wrf(y_mid,x) == slice_lon)
                            idx_a = x; idx_b = x;
                            break;
                        elseif(xdir == 1 && lon_wrf(y_mid,x) > slice_lon)
                    ...
                    y_dir = 1;
                    if(lat_wrf(2,x_mid) < lat_wrf(1,x_mid))
                        y_dir = -1;
                    end
                    for y = 1:ylen_wrf
                        if(lat_wrf(y,x_mid) == slice_lat)
                            idx_a = y; idx_b = y; 
                            break;
                        elseif(y_dir == 1 && lat_wrf(y,x_mid) > slice_lat)
                ...
                if(y_axis)
                    intersection_height_mult = zeros(num_radars,ylen_wrf);
                    ...
                    for rad_idx = 1:num_radars
                        for y = 1:ylen_wrf
                            ...
                            if(rad_idx == 1)
                                for z = 1:zlen_wrf
                                    model_height_slice(y,z) = linear_interpolate(lon_wrf(y,idx_a),lon_wrf(y,idx_b),slice_lon,model_height(y,idx_a,z),model_height(y,idx_b,z));
                    ...
                    intersection_height_mult = zeros(num_radars,xlen_wrf);
                    refl_value_seen = zeros(size(intersection_height_mult));
                    model_height_slice = zeros(size(slice_values));
                    for rad_idx = 1:num_radars
                        for x = 1:xlen_wrf
                            ...
                            if(rad_idx == 1)
                                for z = 1:zlen_wrf
                                    model_height_slice(x,z) = linear_interpolate(lat_wrf(idx_a,x),lat_wrf(idx_b,x),slice_lat,model_height(idx_a,x,z),model_height(idx_b,x,z));
                ...
                %if(use_advanced_br)
                    ...
                    if(y_axis) % Slice is along y-axis
                        intersection_height = zeros(ylen_wrf,1);
                        ...
                        for y = 1:ylen_wrf
                            ...
                            if(max_dbz_idx > 0)
                                intersection_height(y) = linear_interpolate(lon_wrf(y,idx_a),lon_wrf(y,idx_b),slice_lon,radar_height_grid(max_dbz_idx,y,idx_a),radar_height_grid(max_dbz_idx,y,idx_b));
                            end
                        end
                    else % Slice is along x-axis
                        intersection_height = zeros(xlen_wrf,1);
                        intersection_height(:) = NaN;
                        for x = 1:xlen_wrf
                            max_dbz_idx = 0;
                            max_dbz = -999;
                            ...
                            if(max_dbz_idx > 0)
                                intersection_height(x) = linear_interpolate(lat_wrf(idx_a,x),lat_wrf(idx_b,x),slice_lat,radar_height_grid(max_dbz_idx,idx_a,x),radar_height_grid(max_dbz_idx,idx_b,x));
                            end
                        end
                    end
                %end
                ...
                if(limit_slice)
                    if(y_axis)
                        xlim([s_limit n_limit]);
                        ylim([min_height max_height]);
                    else
                        xlim([w_limit e_limit]);
                        ylim([min_height max_height]);
                    end
                end
