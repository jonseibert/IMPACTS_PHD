...
st_name_list = ["KENX","KBUF","KGYX","W-Mobile","BRST"]; % KENX = KALY == ALB
st_lat_list = [0,0,0,43.103,42.9898];
st_lon_list = [0,0,0,-76.193,-73.7070];
num_st_official = 3;
num_st = 5;
...
%800mb AIR:
%   Maximum mixing ratio found: 1.063904e-02 kg/kg
%Latitude: 42.98 (idx 131), Longitude: -73.71 (idx 144)
%
%Intake: (237,136,lowest)
%Burst: (131,144,800mb)
            ...
            for exp_idx = 1:2
                ...
                if(make_wrf_soundings)
                    ...
                    if(exp_idx == 2) % One combo plot per timestamp per member, all experiments
                        ...
                        z = plot([0 0],[0 1100],'Color','#3b3b3b','LineStyle','--','LineWidth',1);
                        hold on;

                        for plot_idx = 1:plot_idx_max
                            num_entries = length(P_storage(plot_idx,~isnan(P_storage(plot_idx,:))));
                            plot(T_storage(plot_idx,1:num_entries),P_storage(plot_idx,1:num_entries),'Color',colors(plot_idx),'LineWidth',3);
                        end

                        xlabel(sprintf('Temperature (%sC)',char(176)));
                        xlim([t_min t_max]);
                        ...
                        for plot_idx = 1:plot_idx_max
                            num_entries = length(P_storage(plot_idx,~isnan(P_storage(plot_idx,:))));
                            plot(MR_storage(plot_idx,1:num_entries),P_storage(plot_idx,1:num_entries),'Color',colors(plot_idx),'LineWidth',3);
                        end
