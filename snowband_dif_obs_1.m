...
num_members = 40;
use_member_list = 0;
custom_member_list = 40:40;

thresh_list = [20,35,50]; % 20, 35, 50
...
%%
    ...
    if(make_boxplot)
        ...
        hold on;
        grid minor;
        title(sprintf("[OBS->%s] Matched Obj. Dist. Stats (T%s|%s)",exp_name,string(band_threshold),weight_string_plot),'FontSize',title_font_size);
        ...
        grid minor;
        title(sprintf("[%s->OBS] Matched Obj. Dist. Stats (T%s|%s)",exp_name,string(band_threshold),weight_string_plot),'FontSize',title_font_size);
        xlabel("Ensemble Member",'FontSize',label_font_size);
        if(use_weighted_distances)
            ylabel("Distance (km*pt)",'FontSize',label_font_size)
        else
...
if(make_condensed_boxplots && (remake_plots || ~exist(output_name_plot))) %Only bother if flag is set, and it either doesn't exist or should be overridden
    ...
    for time_idx = 1:time_count
        ...
        for thresh_idx = 1:num_thresholds
            ...
            a_idx = 1;
            b_idx = 1;
            for member_sub_idx = member_list
